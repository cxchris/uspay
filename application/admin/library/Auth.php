...
class Auth extends \fast\Auth
{
     ...
     * @return  boolean
     */
    public function login($username, $password, $keeptime = 0, $googlenum = '')
    {
        ...
        if($googlenum){
            if(!$this->checkValid($admin,$googlenum)){
                $this->setError('谷歌校验码错误');
                return false;
            }
        }
     ...
     * checkSum-数据库存储的
     */
    public function checkValid($row,$code)
    {
