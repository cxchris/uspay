...
class Cashfree extends Frontend
{
    ...
    public function _initialize(){
        ...
        if($order['channel_type'] != 'cashfree'){
            $this->_error('Cashfree type exception');
        }
    ...
    public function index()
    {
        ...
        $calc = $calc < 0?0:$calc;
        //获取拉起深层链接payload
        $res = Cashfreepay::payload($this->txnToken);
        // Log::record('cashfree拉起深层链接:'.json_encode($res),'notice');

        if($res['code'] != '0000'){
            $this->_error('Order failed');
        }else{
            $data = $res['data']['data']['payload'];
        }
